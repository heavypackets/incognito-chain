...
func (blockchain *BlockChain) storeBurningConfirm(beaconStateDB *statedb.StateDB, beaconBlock *BeaconBlock) error {
	for _, inst := range beaconBlock.Body.Instructions {
		if inst[0] != strconv.Itoa(metadata.BurningConfirmMeta) &&
			inst[0] != strconv.Itoa(metadata.BurningConfirmForDepositToSCMeta) {
			continue
		}
		BLogger.log.Infof("storeBurningConfirm for beaconBlock %d, inst %v, meta type %d", beaconBlock.Header.Height, inst, inst[0])

		txID, err := common.Hash{}.NewHashFromStr(inst[5])
		if err != nil {
			return errors.Wrap(err, "txid invalid")
		}
		if err := statedb.StoreBurningConfirm(beaconStateDB, *txID, beaconBlock.Header.Height); err != nil {
