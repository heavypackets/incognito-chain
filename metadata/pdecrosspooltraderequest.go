...
func (pc PDECrossPoolTradeRequest) ValidateSanityData(chainRetriever ChainRetriever, shardViewRetriever ShardViewRetriever, beaconViewRetriever BeaconViewRetriever, beaconHeight uint64, tx Transaction) (bool, bool, error) {
	...
	if tx.GetType() == common.TxNormalType {
		...
		if err != nil {
			return false, false, errors.New(fmt.Sprintf("Error This is not Tx Burn. Error %v", err))
		}
		if !isBurned {
		...
		if burnedTokenID.String() != common.PRVCoinID.String() {
	...
	if tx.GetType() == common.TxCustomTokenPrivacyType {
		...
		if !ok {
			return false, false, errors.New("This is not custom token transaction")
		}
		...
		isBurned, burnCoin, burnedPRV, err := txToken.GetTxBase().GetTxBurnData()
		if err != nil {
			return false, false, errors.New(fmt.Sprintf("Error This is not Tx Burn. Error %v", err))
		}
		if !isBurned {
		...
		if (pc.TradingFee) != burnCoin.GetValue() {
			return false, false, errors.New("Error Selling amount should be equal to the burned amount")
		}
		...
		if !bytes.Equal(burnedPRV[:], common.PRVCoinID[:]) {
